...
function layout({ title, description, keywords, pagePath, body, schemas = [], current = "", robots = "index, follow, max-image-preview:large", requestHref = "#request" }) {
  ...
  <header class="site-header">
    <div class="container nav">
      ...
      <div class="header-actions">
        ...
        <a class="button button-primary" href="${requestHref}">Заявка</a>
...
function leadForm(context = "Общая заявка") {
  ...
  return `<form class="form-grid" name="lead" method="POST" action="/thanks.html" enctype="multipart/form-data" data-netlify="true" netlify-honeypot="bot-field">
    ...
    <p class="form-note">Нажимая кнопку, вы соглашаетесь с <a href="/privacy.html">политикой конфиденциальности</a> и на обработку персональных данных для подготовки расчета.</p>
  </form>`;
}
...
function privacyPage() {
  ...
  const body = `<section class="page-hero page-hero--compact" style="--hero-image:url('${images.metal}')">
    <div class="container">
      ${breadcrumbs([{ name: "Главная", path: "/" }, { name: "Политика конфиденциальности", path: "/privacy.html" }])}
      <span class="eyebrow">Документы</span>
      <h1>Политика конфиденциальности</h1>
      <p class="lead">Как Техочистка собирает, использует и защищает персональные данные посетителей сайта ${baseUrl.replace("https://", "")}.</p>
    </div>
  </section>
  <section class="section">
    <article class="container legal-doc">
      ...
      <p>Используя Сайт и направляя заявку через форму обратной связи, вы подтверждаете, что ознакомились с Политикой и даёте согласие на обработку персональных данных на условиях, изложенных ниже. Дата публикации текущей редакции: ${updatedAt}</p>

      <h2>1. Оператор персональных данных</h2>
      <p>Оператором персональных данных является компания «Техочистка» (далее — Оператор), оказывающая услуги пескоструйной обработки металла, очистки фасадов, кирпича и дерева.</p>
      <ul class="list">
      ...
      <p>Оператор обрабатывает только те сведения, которые вы самостоятельно направляете через формы Сайта или указываете при обращении по телефону и электронной почте:</p>
      <ul class="list">
      ...
      <p>Оператор не запрашивает паспортные данные, ИНН, платёжные реквизиты и иные специальные категории персональных данных через формы Сайта. Если на фотографии объекта случайно попали люди, просим по возможности направлять снимки без лиц третьих лиц.</p>
      ...
      <ul class="list">
        ...
        <li>связи с вами по телефону или электронной почте по вопросам осмотра, сметы и выполнения работ;</li>
      ...
      <p>Обработка выполняется смешанным способом: с использованием средств автоматизации (формы Сайта, электронная почта, средства связи) и без них (телефонные переговоры, подготовка сметы).</p>
      ...
      <p>Оператор не осуществляет трансграничную передачу персональных данных в качестве самостоятельной цели. Если инфраструктура хостинга находится за пределами Российской Федерации, обработка ограничивается техническим обеспечением работы Сайта.</p>
      ...
      <p>Оператор может обновлять Политику при изменении законодательства, состава обрабатываемых данных или способов работы Сайта. Актуальная версия всегда доступна на странице <a href="/privacy.html">${baseUrl}/privacy.html</a>.</p>
...
const htmlPaths = ["/", ...services.map((service) => service.path), "/portfolio.html", "/privacy.html", ...cities.map((city) => city.path), "/thanks.html"];
